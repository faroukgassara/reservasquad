...
import { EButtonSize, EButtonType, EInputSize, ESize, EVariantLabel, IconComponentsEnum } from '@/Enum/Enum';
...
import { IColorPicker, TColorPickerFormat, TColorPickerOption } from '@/interfaces/IPrimitives/IColorPicker/IColorPicker';
...
const DEFAULT_FALLBACK = colors.black
...
const ColorPicker = ({
    ...
    const handleFormatChange = (nextFormatValue: string | number | (string | number)[]) => {
        ...
        <div className={twMerge('relative w-full max-w-90', className)} ref={containerRef}>
            ...
            {isOpen && !disabled && (
                <div className="absolute left-0 right-0 z-dropdown mt-2 rounded-xxl border border-gray-200 bg-white p-4 shadow-xxl shadow-gray-900/10">
                    ...
                    <ActivePicker
                        className={twMerge('boilerplate-color-picker-widget', pickerClassName)}
                        color={pickerColor}
