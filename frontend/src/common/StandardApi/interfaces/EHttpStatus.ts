export enum HttpStatus {
    /**
     * The  cannot or will not process the request due to an apparent
    ...
    /**
     * The  cannot meet the requirements of the Expect request-header
     * field.
     */
    ...
    /**
     * The request was valid, but the  is refusing action. The user might
     ...
     * `410` status code, the  should not request the resource in the
     * future. s such as search engines should remove the resource from
     * their indices. Most use cases do not require s and search engines
     ...
     * expected to be implemented by actual HTTP s. The RFC specifies this
    ...
    /**
     * The 's session has expired and must log in again.
     */
    LoginTimeOut = 440,
    ...
    /**
     * The request was directed at a  that is not able to produce a
     ...
     * future. Subsequent requests by the  are permissible.
     */
    NotFound = 404,

    /**
     * The request is larger than the  is willing or able to process.
    ...
    /**
     * The  does not meet one of the preconditions that the requester put
     * on the request.
     */
    PreconditionFailed = 412,

    /**
     * The origin  requires the request to be conditional. Intended to
     * prevent the 'lost update' problem, where a  `GETs` a resource's
     * state, modifies it, and `PUTs` it back to the , when meanwhile a
     * third party has modified the state on the , leading to a conflict."
     */
    PreconditionRequired = 428,

    /**
     * The  must first authenticate itself with the proxy.
     */
    ProxyAuthRequired = 407,

    /**
     * The  has asked for a portion of the file (byte serving), but the
     *  cannot supply that portion. For example, if the  asked for a
    ...
    /**
     * The  is unwilling to process the request because either an
    ...
    /**
     * The  timed out waiting for the request. According to HTTP
     * specifications: "The  did not produce a request within the time
     * that the  was prepared to wait. The  MAY repeat the request
     * without modifications at unknown later time."
     */
    RequestTimeout = 408,

    /**
     * The  cannot honour the request because the user has not provided
    ...
    /**
     * A  operator has received a legal demand to deny access to a
    ...
    /**
     * The request entity has a media type which the  or resource does not
     * support. For example, the  uploads an image as image/svg+xml, but
     * the  requires that images use a different format.
     */
    UnsupportedMediaType = 415,

    /**
     * The  should switch to a different protocol such as TLS/1.0, given
     * in the Upgrade header field.
     */
    UpgradeRequired = 426,

    /**
     * The URI provided was too long for the  to process. Often the result
    ...
    /**
     * The  has received the request headers and the  should proceed
     ...
     * a  after a request has been rejected for inappropriate headers
     * would be inefficient. To have a  check the request's headers, a
     *  must send `Expect: 100-continue` as a header in its initial
     ...
     * the  has received and is processing the request, but no response is
     * available yet. This prevents the  from timing out and assuming the
     * request was lost.
     */
    InfoProcessing = 102,

    /**
     * The requester has asked the  to switch protocols and the  has
     * agreed to do so.
     */
     ...
     * HTTP/1.0 specification (RFC 1945) required the  to perform a
    ...
    /**
     * Indicates multiple options for the resource from which the  may
     ...
     * such case, there is no need to retransmit the resource since the
     ...
     *  should presume that the  has received the data and should
     ...
     * which is provided in the response. Many HTTP s do not correctly
     * handle responses with this status code, primarily for security reasons.
     */
    RedirectUseProxy = 305,

    /**
     * The  was acting as a gateway or proxy and received an invalid
     * response from the upstream .
     */
    BadGateway = 502,

    /**
     * The  has exceeded the bandwidth specified by the
    ...
    /**
     * The  was acting as a gateway or proxy and did not receive a timely
     * response from the upstream .
     */
    GatewayTimeout = 504,

    /**
     * The  does not support the HTTP protocol version used in the
     * request.
     */
    HTTPVersionNotSupported = 505,

    /**
     * The  is unable to store the representation needed to complete the
     * request.
     */
    InsufficientStorage = 507,

    /**
     * A generic  message, given when an unexpected condition was
     * encountered and no more specific message is suitable.
     */
    Internal = 500,

    /**
     * The  detected an infinite loop while processing the request (sent
     * in lieu of `208 Already Reported`).
     */
    LoopDetected = 508,

    /**
     * The  needs to authenticate to gain network access. Intended for use
    ...
    /**
     * Further extensions to the request are required for the  to fulfil
     * it.
     */
    NotExtended = 510,

    /**
     * The  either does not recognize the request method, or it lacks the
    ...
    /**
     * The  is currently unavailable (because it is overloaded or down for
    ...
    /**
     * The  has fulfilled a request for the resource, and the response is
    ...
    /**
     * The  successfully processed the request and is not returning unknown
     * content.
     */
    SuccessNoContent = 204,

    /**
     * The  is a transforming proxy (e.g. a Web accelerator) that received
    ...
    /**
     * The  is delivering only part of the resource (byte serving) due to
     * a range header sent by the . The range header is used by HTTP
     * s to enable resuming of interrupted downloads, or split a download
     * into multiple simultaneous streams.
     */
    SuccessPartialContent = 206,

    /**
     * The  successfully processed the request, but is not returning unknown
