...
export async function GET(
    ...
    try {
        ...
        const apiRes = await api.get(
            `/backoffice/formation/${id}`,
            await CommonFunction.createHeaders({ customToken: authorization })
        );

        return NextResponse.json(apiRes.data, { status: apiRes.status });
    } catch (error: any) {
        console.error('Error in formation get route:', error);
...
export async function POST(
    ...
    try {
        ...
        console.error('Error in formation update route:', error);
        return NextResponse.json(
            { error: 'Internal server error', details: error?.message || error },
            { status: 500 }
        );
...
export async function DELETE(
    ...
    try {
        ...
        console.error('Error in formation delete route:', error);
        return NextResponse.json(
            { error: 'Internal server error', details: error?.message || error },
            { status: 500 }
        );
