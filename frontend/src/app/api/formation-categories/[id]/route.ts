...
export async function POST(
    ...
    try {
        ...
        const apiRes = await api.post(
            `/backoffice/formation-category/${id}`,
            body,
        ...
        console.error('Error in formation category update route:', error);
...
export async function DELETE(
    ...
    try {
        ...
        console.error('Error in formation category delete route:', error);
        return NextResponse.json(
            { error: 'Internal server error', details: error?.message || error },
            { status: 500 }
        );
